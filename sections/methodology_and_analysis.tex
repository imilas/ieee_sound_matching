...
We systematically evaluate iterative sound-matching pipelines that combine
different differentiable loss functions with differentiable synthesizers.
By ``performance'' we mean the similarity of the synthesized output to a
target, assessed via P-Loss, MSS, and---most importantly---manual listening
tests. This setup allows us to test our central hypothesis: that the relative
performance of a loss function is contingent on the synthesis method, and
thus no universal ``best'' similarity measure exists.

We evaluate four loss functions (Section~\ref{sec:loss_implementation})
