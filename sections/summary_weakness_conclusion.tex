...
\subsection{Illustrative Loss Landscapes}

To complement our quantitative, statistically robust analysis, we provide qualitative examples of loss landscapes to illustrate why specific losses succeed or fail in particular synthesis contexts. These examples should not be seen as conclusive evidence, but rather as illustrative aids to understanding the mechanisms behind the numerical results.

Figure~\ref{fig:loss_landscape_noisebp} shows the case of \BPNoise{} modified to use only a high-pass filter parameter. Here, \LoneSpec{}, \SIMSESpec{}, and \JTFS{} exhibit clear global minima near the correct parameter (red dashed line), whereas \DTWEnv{} remains relatively flat around the target, making gradient-based exploration more challenging. This observation explains DTW’s weaker performance in this context.

In contrast, Figure~\ref{fig:loss_landscape_ampmod} depicts a simplified version of \AmpMod{} with only the amplitude-modulation parameter (range: 0.1–20\,Hz). In this case, \DTWEnv{} yields the smoothest and most informative landscape, strongly correlating with proximity to the target. This aligns with its superior performance in amplitude-modulated synthesis observed in our experiments.

These examples underscore the importance of loss landscapes in shaping optimization dynamics and further motivate future work on landscape-aware loss design.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/experiment_plots/comparing_loss_landscapes_normalized.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/experiment_plots/comparing_lanscapes_1_1d_normalized.png}
